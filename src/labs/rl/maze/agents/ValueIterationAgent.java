    ...
    extends StochasticAgent
{
    ...
    public void valueIteration(StateView state)
    {
        ...
        Map<Coordinate, Double> initialUtilities = this.getZeroMap(state); // Empty starting map
        Map<Coordinate, Double> newUtilities = this.getZeroMap(state); // Empty starting map

        double error = Double.POSITIVE_INFINITY; // Max error between steps 

        while (error > (EPSILON * (1 - GAMMA) / GAMMA)) {
            error = 0.0; // Reset error
            initialUtilities = newUtilities; // Update utilities

            newUtilities = this.getZeroMap(state); // Empty starting map

            // Loop through states
            for (Coordinate c : initialUtilities.keySet()) {
                if (isTerminalState(c)) {
                    newUtilities.put(c, RewardFunction.getReward(c)); // Set terminal states to their rewards
                } else {
                    double maxUtility = Double.NEGATIVE_INFINITY; // Max utility for this state

                    // Loop through actions
                    for (Direction d : TransitionModel.CARDINAL_DIRECTIONS) {
                        double thisActionUtility = 0.0; // Utility for this action

                        // Loop through possible results
                        for (Pair<Coordinate, Double> transition : TransitionModel.getTransitionProbs(state, c, d)) {
                            thisActionUtility += transition.getSecond() * initialUtilities.get(transition.getFirst()); // Probability of ending up at s'|s,a * utility of s'
                        }

                        maxUtility = Math.max(maxUtility, thisActionUtility); // Update max utility
                    }

                    // Update new utilities
                    newUtilities.put(c, RewardFunction.getReward(c) + (GAMMA * maxUtility));
                }

                // Update error
                error = Math.max(error, Math.abs(newUtilities.get(c) - initialUtilities.get(c)));
            }

        }

        // Print utilities and coordinates
        for (Coordinate c : newUtilities.keySet()) {
            System.out.println(c + " : " + newUtilities.get(c));
        }

        this.setUtilities(initialUtilities); // Set utilities to the final utilities
