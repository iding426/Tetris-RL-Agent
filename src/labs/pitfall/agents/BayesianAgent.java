    ...
    extends Agent
{
        ...
        extends Object
    {
        ...
        public Coordinate getNextCoordinateToExplore()
        {
            ...
            /*
             * Notes:
             * -- Loop over each frontier square
             * -- Set our query to either Pit == True or Pit == False (Only need to do one, we just choose highest or smallest probability)
             * -- For each query square, iterate across all other combinations of frontier pits
             * -- If the frontier combination matches the breezes then we should consider it as a possibility (multiply p*p*(1-p) etc across orientation)
             * -- Sum the probabilities that they are true to find probability sum for this query
             * -- Do this for all frontier squares 
             * -- Choose one that is least likely to contain a 
             * -- Generate frontier possibilities with bitstrings rather than creating them and storing in a list (which is going to be too long)
             */

            boolean[] frontier = new boolean[frontierPitCoordinates.size()];
            Coordinate toExplore = null;
            double minProb = Double.MAX_VALUE;

            // Map coordinate indices to a coordinate in frontier and vice versa
            Map<Integer, Coordinate> indexToCoord = new HashMap<Integer, Coordinate>();
            Map<Coordinate, Integer> coordToIndex = new HashMap<Coordinate, Integer>();

            int index = 0;
            for (Coordinate coord : frontierPitCoordinates) {
                indexToCoord.put(index, coord);
                coordToIndex.put(coord, index);
                index++;
            }

            // Query each frontier square
            for (int i = 0; i < frontier.length; i++) {
                ...
                // Generate all combinations of frontiers given that i is true
                for (int j = 0; j < Math.pow(2, frontier.length - 1); j++) {
                    // Generate one combination
                    int bit = 0;
                    for (int k = 0; k < frontier.length; k++) {
                        if (k != i) { // Skip the index we've already set to true
                            frontier[k] = ((j & (1 << bit)) != 0);
                            bit++;
                        }
                    }

                    // Validate this frontier possibility and calculate its probability
                    boolean valid = true; 

                    // Loop over frontier
                    for (int k = 0; k < frontier.length; k++) {
                        if (frontier[k]) {
                            // This means that the frontier square is a pit
                            // All neighboring explored squares should contain a breeze
                            for (Coordinate n : indexToCoord.get(k).getAdjacentCoordinatesInCardinalDirections()) {
                                // If explored it must contain a breeze
                                if (knownBreezeCoordinates.containsKey(n) && !knownBreezeCoordinates.get(n)) {
                                    valid = false;
                                    break;
                                }
                            }
                            ...
                            // This frontier has no pit, check all neighbors and make sure all neighboring breezes are actually next to a pit
                            for (Coordinate n : indexToCoord.get(k).getAdjacentCoordinatesInCardinalDirections()) {
                                // If explored and contains a breeze check its neighbors
                                if (this.knownBreezeCoordinates.containsKey(n) && this.knownBreezeCoordinates.get(n) == true) {
                                    boolean neighboringPit = false;

                                    for (Coordinate neighbor: n.getAdjacentCoordinatesInCardinalDirections()) {
                                        if (frontierPitCoordinates.contains(neighbor)) {
                                            int ind = coordToIndex.get(neighbor);
                                            if (frontier[ind]) {
                                                neighboringPit = true;
                                                break;
                                            }
                    ...
                    if (valid) {
                        ...
                        // Add to probability for this query 
                        currProb += prob; 
                    }
                }
